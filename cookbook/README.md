# ARKITEKT Cookbook

> Definitive reference guides for ARKITEKT development.

## Guides

| Guide | Description |
|-------|-------------|
| [QUICKSTART.md](QUICKSTART.md) | **Start here** - Create a new app in 5 minutes |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Framework vs scripts, Clean Architecture, layer organization |
| [CONVENTIONS.md](CONVENTIONS.md) | Naming conventions, file organization, module patterns |
| [API_DESIGN_PHILOSOPHY.md](API_DESIGN_PHILOSOPHY.md) | When to match ImGui vs when to improve |
| [WIDGETS.md](WIDGETS.md) | Widget development patterns and API contract |
| [ARKCONTEXT.md](ARKCONTEXT.md) | Frame-scoped context, caching, and Disabled Stack |
| [THEME_MANAGER.md](THEME_MANAGER.md) | Dynamic theming system with REAPER integration |
| [STORAGE.md](STORAGE.md) | Persistence options: ExtState, JSON, Settings |
| [SHORTCUTS.md](SHORTCUTS.md) | Keyboard input handling patterns |
| [TESTING.md](TESTING.md) | Test runner, assertions, integration testing |
| [LUA_PERFORMANCE_GUIDE.md](LUA_PERFORMANCE_GUIDE.md) | Lua/REAPER/ImGui performance optimization |
| [REFACTOR_PLAN.md](REFACTOR_PLAN.md) | Systematic refactoring methodology and patterns |
| [MODULARISATION.md](MODULARISATION.md) | When and how to extract reusable code |
| [TODO_GUIDE.md](TODO_GUIDE.md) | Creating and organizing TODO tasks with AI collaboration |
| [DEPRECATED.md](DEPRECATED.md) | Deprecation tracker and removal process |

## Quick Links

- [CLAUDE.md](../CLAUDE.md) - AI assistant quick start
- [TODO/](../TODO/) - Actionable improvement tasks
