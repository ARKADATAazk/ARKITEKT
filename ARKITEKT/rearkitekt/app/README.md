# rearkitekt/app — Framework Application Layer
...
**API:**
```lua
local Titlebar = require('rearkitekt.app.chrome.titlebar.titlebar')
...
**App icon drawing functions.** Three ReArkitekt logo variations (DPI-aware vector graphics).

**API:**
```lua
local Icon = require('rearkitekt.app.assets.icon')
Icon.draw_rearkitekt(ctx, x, y, size, color)     -- Original (smaller circles)
Icon.draw_rearkitekt_v2(ctx, x, y, size, color)  -- Refined (larger bulbs, faders)
...
- **Changed:** Entry points from `require('rearkitekt.app.init')` to `dofile()` pattern
...
- `rearkitekt.core.colors` - Color utilities
- `rearkitekt.core.config` - Config merging utilities
- `rearkitekt.core.settings` - Persistent settings
- `rearkitekt.gui.style.*` - Style presets
