...
function M.render_complete_tile(ctx, dl, rect, item_data, tile_state, base_color, animator, visualization, cache_mgr, is_disabled)
    ...
    if item_data.is_midi then
      if visualization.DisplayMidiItemTransparent then
        ...
        if not thumbnail and visualization.GetMidiThumbnail then
          thumbnail = visualization.GetMidiThumbnail(ctx, item_data.cache, item_data.item)
        end
        if thumbnail then
          ImGui.SetCursorScreenPos(ctx, x1, content_y1)
          ImGui.Dummy(ctx, tile_w, y2 - content_y1)
      ...
      if visualization.DisplayWaveformTransparent then
        ...
        if waveform then
          visualization.DisplayWaveformTransparent(ctx, waveform, dark_color, dl, tile_w)
        end
      end
    end
  end
