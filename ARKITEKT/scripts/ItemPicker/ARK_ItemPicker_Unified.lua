-- @noindex
-- ItemPicker Unified Daemon - Always-on background process with instant UI
-- Combines daemon functionality with instant show/hide overlay
...
local function log(msg)
  reaper.ShowConsoleMsg("[ItemPicker Unified] " .. msg .. "\n")
end
...
-- ============================================================================
-- Background Processing (from original daemon)
-- ============================================================================
...
local function initialize()
  log("Initializing unified daemon...")
  ...
  daemon.ctx = ImGui.CreateContext("Item Picker Unified")
  ...
  log("Initialization complete - ready for instant show/hide")
end
...
local function main_loop()
  ...
  -- Check for toggle requests from second instance
...
-- Check if daemon is already running via extended state
local ext_state_section = "ARK_ItemPicker_Unified"
local ext_state_key = "daemon_running"
local ext_state_toggle_key = "toggle_request"

local function check_toggle_request()
  local toggle_req = reaper.GetExtState(ext_state_section, ext_state_toggle_key)
  if toggle_req ~= "" then
    -- Clear the toggle request
    reaper.SetExtState(ext_state_section, ext_state_toggle_key, "", false)
    -- Toggle UI
    toggle_ui()
  end
end
...
local already_running = reaper.GetExtState(ext_state_section, ext_state_key) == "1"

if already_running then
  -- Signal the running instance to toggle
  reaper.SetExtState(ext_state_section, ext_state_toggle_key, "1", false)
  log("Toggling UI...")
  return
end

-- Mark as running
reaper.SetExtState(ext_state_section, ext_state_key, "1", false)
...
-- User can toggle to show/hide instantly
daemon.ui_visible = false

-- Register cleanup
reaper.atexit(function()
  reaper.DeleteExtState(ext_state_section, ext_state_key, false)
  reaper.DeleteExtState(ext_state_section, ext_state_toggle_key, false)
  cleanup()
end)
