...
function M.SettingsButton_new(config)
  return setmetatable({
    config = config or {},
    hover_alpha = 0,
  }, SettingsButton)
...
function SettingsButton:draw_icon(ctx, dl, x, y)
  local color = self.config.icon_color or hexrgb("#CCCCCC")
  local center_x = x + 10
  local center_y = y + 10
  local radius = 6
  ...
  -- Draw gear/cog icon
  -- Center circle
  ImGui.DrawList_AddCircleFilled(dl, center_x, center_y, 3, color)

  -- Gear teeth (8 small rectangles around the circle)
  for i = 0, 7 do
    local angle = (i * 45) * (3.14159 / 180)
    local tooth_x = center_x + (radius * math.cos(angle))//1
    local tooth_y = center_y + (radius * math.sin(angle))//1
    ImGui.DrawList_AddRectFilled(dl, tooth_x - 1, tooth_y - 1, tooth_x + 1, tooth_y + 1, color, 0)
  end
end

function SettingsButton:draw(ctx, x, y, on_click, use_foreground_drawlist)
  local dl = use_foreground_drawlist and ImGui.GetForegroundDrawList(ctx) or ImGui.GetWindowDrawList(ctx)
  local cfg = self.config
  local btn_size = cfg.size or 32
  ...
  local icon_x = (x + (btn_size - 20) / 2 + 0.5)//1
  local icon_y = (y + (btn_size - 20) / 2 + 0.5)//1
  self:draw_icon(ctx, dl, icon_x, icon_y)
  ...
  if use_foreground_drawlist then
    ...
    ImGui.SetCursorScreenPos(ctx, x, y)
    ImGui.InvisibleButton(ctx, "##settings_button", btn_size, btn_size)
  ...
  if is_hovered then
    local tooltip = "Settings (coming soon)"
    Tooltip.show(ctx, tooltip)
  end
...
function SettingsButton:lerp_color(a, b, t)
  local ar, ag, ab, aa = (a >> 24) & 0xFF, (a >> 16) & 0xFF, (a >> 8) & 0xFF, a & 0xFF
  local br, bg, bb, ba = (b >> 24) & 0xFF, (b >> 16) & 0xFF, (b >> 8) & 0xFF, b & 0xFF
...
-- Quantize options centralized in Region_Playlist/core/config.lua (M.QUANTIZE.options)
