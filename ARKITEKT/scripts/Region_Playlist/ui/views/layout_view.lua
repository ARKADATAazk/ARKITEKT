-- @noindex
-- Region_Playlist/ui/views/layout_view.lua
...
function LayoutView:draw(ctx, region_tiles)
  ...
  if self.state.get_layout_mode() == 'horizontal' then
    self:draw_horizontal(ctx, region_tiles, display_playlist, pool_data)
  else
    self:draw_vertical(ctx, region_tiles, display_playlist, pool_data)
  end
end

function LayoutView:draw_horizontal(ctx, region_tiles, display_playlist, pool_data)
  ...
  region_tiles:draw_active(ctx, display_playlist, active_height)
  ...
  region_tiles:draw_pool(ctx, pool_data, pool_height)
...
function LayoutView:draw_vertical(ctx, region_tiles, display_playlist, pool_data)
  local content_w, content_h = ImGui.GetContentRegionAvail(ctx)
  ...
  if ImGui.BeginChild(ctx, "##left_column", active_width, content_h, ImGui.ChildFlags_None, 0) then
    region_tiles:draw_active(ctx, display_playlist, content_h)
  end
  ImGui.EndChild(ctx)
  ...
  if ImGui.BeginChild(ctx, "##right_column", pool_width, content_h, ImGui.ChildFlags_None, 0) then
    region_tiles:draw_pool(ctx, pool_data, content_h)
  end
  ImGui.EndChild(ctx)
