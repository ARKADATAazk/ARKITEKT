...
function M.render_region(ctx, dl, rect, region, opts)
  ...
  local badge_margin = opts.badge_margin or M.DEFAULTS.badge_margin
  if responsive.show_badge then
    local badge_rect = {
      tile_rect.x1,
      tile_rect.y1,
      tile_rect.x2 - badge_margin,
      tile_rect.y2,
    }
    badge = Base.draw_repeat_badge(ctx, dl, badge_rect, opts.reps or region and region.reps or 1, opts.badge_enabled ~= false)
  end

  local text = nil
  if responsive.show_text then
    local bounds = compute_text_bounds(tile_rect, badge.visible and badge.rect or nil, opts.text_margin_right or M.DEFAULTS.text_margin_right)
...
function M.render_playlist(ctx, dl, rect, playlist, opts)
  ...
  local base_color = opts.base_color or (opts.state and opts.state.selected and Colors.tile_background_selected) or Colors.tile_background
  local background = Base.draw_tile_background(dl, rect, base_color, opts.state)
  local tile_rect = ensure_rect(background.rect, rect)
  local tile_height = opts.tile_height or math.max(0, (tile_rect.y2 or 0) - (tile_rect.y1 or 0))
  local responsive = Base.calculate_responsive_elements(tile_height)

  local text_value = playlist and playlist.name or (opts and opts.label)
  ...
  if responsive.show_text then
    ...
    text = Base.draw_text_with_truncation(ctx, dl, text_value, bounds)

    if text and playlist and playlist.chip_color and text.commands and text.commands[1] then
      text.commands[1].args[3] = playlist.chip_color
    end
  end

  local badge = { visible = false, commands = {} }
  if responsive.show_badge then
    local count = (playlist and playlist.items and #playlist.items) or opts.item_count
    if count and count > 0 then
      ...
      badge = Base.draw_repeat_badge(ctx, dl, badge_rect, count, true)
      if badge.commands and badge.commands[2] then
        badge.commands[2].args[4] = string.format('[%d]', count)
