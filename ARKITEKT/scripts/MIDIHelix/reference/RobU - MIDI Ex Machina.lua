...
function Init()
	...
	-- Check if there is at least one selected media item
	if itemCount > 0 then
		-- Get the first selected media item
		local item = reaper.GetSelectedMediaItem(0, 0)
		-- Get the active take from the selected media item
		m.activeTake = reaper.GetActiveTake(item)
	else
		-- No items selected, handle appropriately
		--reaper.ShowMessageBox("No MIDI items are selected.", "Error", 0)
		m.activeTake = nil -- or handle as needed
	end
