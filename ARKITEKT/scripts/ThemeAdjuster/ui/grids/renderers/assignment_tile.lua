...
function M.render(ctx, rect, item, state, view, tab_id)
  ...
  -- Tile background
  local bg_color = hexrgb("#1A1A22")

  -- Highlight if being dragged or hovered
  if state.is_dragged then
    bg_color = hexrgb("#252535")
  elseif state.is_hovered and not state.is_dragged then
    bg_color = hexrgb("#1F1F2A")
  end
