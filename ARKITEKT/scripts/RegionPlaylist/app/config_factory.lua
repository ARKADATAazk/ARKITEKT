-- @noindex
-- RegionPlaylist/app/config_factory.lua
-- Config factory functions for dynamic configurations
--
-- Factory pattern: Single-call functions that create configured objects.
-- Uses runtime dependencies (callbacks, state modules) to build configs.
...
function M.get_transport_config(state_module)
  ...
  return {
    ...
    corner_buttons = {
      ...
      bottom_left = create_viewmode_button(state_module),
      bottom_right = {
        custom_draw = function(ctx, dl, x, y, width, height, is_hovered, is_active, color)
