...
local function draw_stack_layers(ctx, dl, x1, y1, x2, y2, track_count, chip_color)
  ...
  for i = num_layers, 1, -1 do
    ...
    if chip_color then
      local cr, cg, cb = Ark.Colors.RgbaToComponents(chip_color)
      local br, bg_c, bb = Ark.Colors.RgbaToComponents(STACK_BASE)
      ...
      layer_bg = Ark.Colors.ComponentsToRgba(r, g, b, math.floor(255 * alpha_factor))
    else
      layer_bg = Ark.Colors.WithAlpha(STACK_BASE, math.floor(255 * alpha_factor))
    end
    ...
    ImGui.DrawList_AddRect(dl, lx1, ly1, lx2, ly2, Ark.Colors.WithAlpha(layer_border, math.floor(180 * alpha_factor)), rounding, 0, 1)
  end
end
...
function M.render(ctx, rect, template, state, metadata, animator)
  ...
  if chip_color then
    local cr, cg, cb = Ark.Colors.RgbaToComponents(chip_color)
    local br, bg_c, bb = Ark.Colors.RgbaToComponents(BG_BASE)
    ...
    bg_color = Ark.Colors.ComponentsToRgba(r, g, b, 255)
  end

  if hover_factor > 0.01 then
    local r1, g1, b1 = Ark.Colors.RgbaToComponents(bg_color)
    local r2, g2, b2 = Ark.Colors.RgbaToComponents(BG_HOVER)
  ...
  if state.selected then
    ...
    if chip_color then
      local cr, cg, cb = Ark.Colors.RgbaToComponents(chip_color)
      local br, bg_c, bb = Ark.Colors.RgbaToComponents(BRD_BASE)
      local r = math.floor(br * (1 - color_blend) + cr * color_blend)
      local g = math.floor(bg_c * (1 - color_blend) + cg * color_blend)
      local b = math.floor(bb * (1 - color_blend) + cb * color_blend)
      border_color = Ark.Colors.ComponentsToRgba(r, g, b, 255)
    end

    if hover_factor > 0.01 then
      local r1, g1, b1 = Ark.Colors.RgbaToComponents(border_color)
      local r2, g2, b2 = Ark.Colors.RgbaToComponents(BRD_HOVER)
  ...
  -- Template name
  local name_color = Ark.Colors.WithAlpha(hexrgb('#CCCCCC'), text_alpha)  -- Match Parameter Library text color
  ...
  if tile_h >= M.CONFIG.hide_path_below and template.relative_path ~= '' then
    ...
    local path_color = Ark.Colors.WithAlpha(hexrgb('#A0A0A0'), path_alpha)
  ...
  if is_favorite then
    ...
    if chip_color then
      ...
      star_color = Ark.Colors.ComponentsToRgba(r, g, b, is_star_hovered and 160 or 80)
    else
