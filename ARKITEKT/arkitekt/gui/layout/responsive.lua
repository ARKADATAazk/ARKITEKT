...
function M.calculate_scaled_rounding(tile_height, base_height, min_height, responsive_config)
  local rounding_config = responsive_config and responsive_config.rounding_scaling
  if not rounding_config or not rounding_config.enabled then
    return rounding_config and rounding_config.max_rounding or 6
  end

  local min_rounding = rounding_config.min_rounding or 0
  local max_rounding = rounding_config.max_rounding or 6

  local height_range = base_height - min_height
  if height_range <= 0 then return max_rounding end

  local height_factor = (tile_height - min_height) / height_range
  height_factor = min(1.0, max(0.0, height_factor))

  local scaled_rounding = min_rounding + (max_rounding - min_rounding) * height_factor
  return max(min_rounding, (scaled_rounding)//1)
end
...
function M.calculate_responsive_tile_height(opts)
  ...
  -- Calculate base rounding for non-responsive case
  local base_rounding = M.calculate_scaled_rounding(base_tile_height, base_tile_height, min_tile_height, responsive_config)

  if not responsive_config.enabled or item_count == 0 then
    return base_tile_height, base_gap, base_rounding
  end
  ...
  if available_for_tiles <= 0 then return base_tile_height, base_gap, base_rounding end
  ...
  local final_rounding = M.calculate_scaled_rounding(final_height, base_tile_height, min_tile_height, responsive_config)

  return final_height, final_gap, final_rounding
end
