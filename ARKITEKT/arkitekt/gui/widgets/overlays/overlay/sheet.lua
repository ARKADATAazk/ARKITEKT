...
function Sheet.render(ctx, alpha, bounds, content_fn, opts)
  ...
  local bg = Colors.WithAlpha(bg_base, Colors.Opacity(config.sheet.background.opacity * alpha))
  local dl = bounds.dl
  ...
  Draw.RectFilled(dl, x, y, x+w, y+h, bg, r)
  ...
  if config.sheet.gradient.top_enabled then
    ...
    for i = 0, gradient_height, 2 do
      ...
      local grad_color = Colors.WithAlpha(config.sheet.gradient.top_color, grad_alpha)
      Draw.RectFilled(dl, x, y + i, x + w, y + i + 2, grad_color, 0)
    end
  end
  ...
  if config.sheet.gradient.bottom_enabled then
    ...
    for i = 0, bottom_gradient_height, 2 do
      ...
      local grad_color = Colors.WithAlpha(config.sheet.gradient.bottom_color, grad_alpha)
      Draw.RectFilled(dl, x, y + h - bottom_gradient_height + i, x + w, y + h - bottom_gradient_height + i + 2, grad_color, 0)
    end
  end
  
  local border_color = Colors.WithAlpha(
  ...
  local inner_border = Colors.WithAlpha(
  ...
  if #title > 0 then
    local title_color = Colors.WithAlpha(
    ...
    for i = 0, divider_gradient_w do
      ...
      local div_color = Colors.WithAlpha(config.sheet.header.divider_color, div_alpha)
      Draw.Line(dl, x + i, divider_y, x + i, divider_y, div_color, config.sheet.header.divider_thickness)
    end

    local main_divider = Colors.WithAlpha(config.sheet.header.divider_color, divider_base_alpha)
    ...
    local highlight = Colors.WithAlpha(
