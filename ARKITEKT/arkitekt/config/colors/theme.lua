...
-- DSL WRAPPERS
-- =============================================================================
-- 2-Zone DSL - transitions at midpoint (t=0.5):
--   snap2(dark, light)       - discrete snap at midpoint
--   lerp2(dark, light)       - smooth interpolation
--   offset2(dark, [light])   - delta from BG_BASE (light defaults to dark)
...
local function snap2(dark_val, light_val)
  return { mode = 'snap2', dark = dark_val, light = light_val }
end

local function lerp2(dark_val, light_val)
  return { mode = 'lerp2', dark = dark_val, light = light_val }
end

local function offset2(dark_delta, light_delta)
  return { mode = 'offset2', dark = dark_delta, light = light_delta or dark_delta }
end
...
--   offset2  → BG-derived (apply delta to BG_BASE)
--   snap2    → hex strings = color, numbers = value
--   lerp2    → hex strings = color, numbers = value
--   offset3/snap3/lerp3 → 3-zone variants

M.colors = {
  -- === BACKGROUNDS (from BG_BASE) ===
  BG_BASE         = bg(),
  BG_HOVER        = offset2(0.03, -0.06),   -- Light: more contrast (was -0.04)
  BG_ACTIVE       = offset2(0.05, -0.10),   -- Light: more contrast (was -0.07)
  ...
  BORDER_INNER    = offset2(0.05, -0.03),
  BORDER_HOVER    = offset2(0.18, -0.14),   -- was 0.10/-0.08, increased for visible pop
  BORDER_ACTIVE   = offset2(0.30, -0.22),   -- was 0.15/-0.12, much brighter on click
  BORDER_FOCUS    = offset2(0.25, -0.18),   -- was 0.20/-0.15
  BORDER_OUTER    = snap2(0x000000FF, 0x404040FF),
  BORDER_OUTER_OPACITY = lerp2(0.87, 0.60),

  -- === ACCENTS (from BG_BASE) ===
  ACCENT_PRIMARY       = offset2(0.15, -0.12),
  ACCENT_TEAL          = offset2(0.15, -0.12),
  ACCENT_TEAL_BRIGHT   = offset2(0.25, -0.20),
  ACCENT_TRANSPARENT   = offset2(0.15, -0.12),
  ACCENT_TRANSPARENT_OPACITY = lerp2(0.67, 0.67),
  ...
  ACCENT_WHITE        = offset2(0.12, -0.15),
  ACCENT_WHITE_BRIGHT = offset2(0.20, -0.22),
  ACCENT_SUCCESS = lerp2(0x4CAF50FF, 0x2E7D32FF),
  ACCENT_WARNING = lerp2(0xFFA726FF, 0xF57C00FF),
  ACCENT_DANGER  = lerp2(0xEF5350FF, 0xC62828FF),

  -- === TEXT ===
  TEXT_NORMAL = snap2(0xFFFFFFFF, 0x000000FF),
  TEXT_HOVER  = snap2(0xF0F0F0FF, 0x1A1A1AFF),
  TEXT_ACTIVE = snap2(0xE8E8E8FF, 0x222222FF),
  ...
  TILE_NAME_COLOR      = snap2(0xDDE3E9FF, 0x1A1A1AFF),
  TILE_FILL_BRIGHTNESS = lerp2(0.25, 0.35), -- Normalized: 0.5=1x, so 0.25→0.5x, 0.35→0.7x
  TILE_FILL_SATURATION = lerp2(0.20, 0.35), -- Normalized: 0.5=1x, so 0.20→0.4x, 0.35→0.7x
  TILE_FILL_OPACITY    = lerp2(0.4, 0.6),   -- Light: slightly more opaque (was 0.5)

  -- === BADGES ===
  BADGE_BG             = snap2(0x14181CFF, 0xE8ECF0FF),
  BADGE_TEXT           = snap2(0xFFFFFFFF, 0x1A1A1AFF),
  BADGE_BG_OPACITY     = lerp2(0.85, 0.90),
  BADGE_BORDER_OPACITY = lerp2(0.20, 0.15),

  -- === PLAYLIST ===
  PLAYLIST_TILE_COLOR  = snap2(0x3A3A3AFF, 0xA0A0A0FF),  -- Light: darker for contrast (was #D0D0D0)
  PLAYLIST_NAME_COLOR  = snap2(0xCCCCCCFF, 0x2A2A2AFF),
  PLAYLIST_BADGE_COLOR = snap2(0x999999FF, 0x666666FF),

  -- === OPERATIONS (drag/drop feedback) ===
  OP_MOVE   = snap2(0xCCCCCCFF, 0x444444FF),  -- Gray - move operation
  OP_COPY   = snap2(0x06B6D4FF, 0x0891B2FF),  -- Cyan - copy operation
  OP_DELETE = snap2(0xE84A4AFF, 0xDC2626FF),  -- Red - delete operation
  OP_LINK   = snap2(0x4A9EFFFF, 0x2563EBFF),  -- Blue - link/reference

  -- === COLORED BUTTONS ===
  -- Each variant: BG (base), HOVER (+lightness), ACTIVE (-lightness), TEXT (auto contrast)

  -- Danger (red)
  BUTTON_DANGER_BG     = lerp2(0xB91C1CFF, 0xFCA5A5FF),
  BUTTON_DANGER_HOVER  = lerp2(0xDC2626FF, 0xFEE2E2FF),
  BUTTON_DANGER_ACTIVE = lerp2(0x991B1BFF, 0xF87171FF),
  BUTTON_DANGER_TEXT   = snap2(0xFFFFFFFF, 0x7F1D1DFF),

  -- Success (green)
  BUTTON_SUCCESS_BG     = lerp2(0x15803DFF, 0x86EFACFF),
  BUTTON_SUCCESS_HOVER  = lerp2(0x16A34AFF, 0xBBF7D0FF),
  BUTTON_SUCCESS_ACTIVE = lerp2(0x166534FF, 0x4ADE80FF),
  BUTTON_SUCCESS_TEXT   = snap2(0xFFFFFFFF, 0x14532DFF),

  -- Warning (amber/orange)
  BUTTON_WARNING_BG     = lerp2(0xB45309FF, 0xFCD34DFF),
  BUTTON_WARNING_HOVER  = lerp2(0xD97706FF, 0xFDE68AFF),
  BUTTON_WARNING_ACTIVE = lerp2(0x92400EFF, 0xFBBF24FF),
  BUTTON_WARNING_TEXT   = snap2(0xFFFFFFFF, 0x78350FFF),

  -- Info (blue)
  BUTTON_INFO_BG     = lerp2(0x1D4ED8FF, 0x93C5FDFF),
  BUTTON_INFO_HOVER  = lerp2(0x2563EBFF, 0xBFDBFEFF),
  BUTTON_INFO_ACTIVE = lerp2(0x1E40AFFF, 0x60A5FAFF),
  BUTTON_INFO_TEXT   = snap2(0xFFFFFFFF, 0x1E3A8AFF),
}
